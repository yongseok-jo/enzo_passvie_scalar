...
#define GMC_TEST //#####
    ...
    // if MBHFeedback = 4 to 6, provide the jet direction with a random noise
    if (MBHFeedback >= 4 && MBHFeedback <= 6) {

      float theta, phi, MaximumNoiseAngle;
      if (MBHFeedback == 4 || MBHFeedback == 6) 
	MaximumNoiseAngle = 10.0;
      if (MBHFeedback == 5) 
	MaximumNoiseAngle = 90.0; //launched in random direction
...
//	   rho_jet, rho_metal_jet, rho_colour_jet, metallicity_edge); 
    ...
    /* When MBHFeedback = 6 (radio-/quasar-mode duality) 
       change the power of the mechanical feedback based on the accretion rate;
       check Star_ComputePhotonRates.C for the change in the radiative feedback */

    const double Grav = 6.673e-8, m_h = 1.673e-24, sigma_T = 6.65e-25;
    float mdot_Edd = 4.0 * PI * Grav * cstar->Mass * m_h /
      max(MBHFeedbackRadiativeEfficiency, 0.1) / sigma_T / c;  //same as in Star_CalculateMassAccretion

    if (MBHFeedback == 6) { 
      MBHJetsVelocity *= 
	sqrt(1 - 1 / (1 + exp(-50*(cstar->last_accretion_rate/mdot_Edd - 0.1)))); //#####
      fprintf(stdout, "grid::AddFS: mdot, mdot_Edd, radio-/quasar-mode factor = %g %g %g\n", 
	      cstar->last_accretion_rate, mdot_Edd, 	      
	      sqrt(1 - 1 / (1 + exp(-50*(cstar->last_accretion_rate/mdot_Edd - 0.1)))));
    }
