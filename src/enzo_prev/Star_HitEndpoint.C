...
int Star::HitEndpoint(FLOAT Time)
{
  ...
  case MBH:
#ifdef GMC_TEST  //#####  THIS HAD NO IMPACT...  You stupid...  (check the first if statement in this file -> particle type stayed the same but never listed as Star after first timestep, 12/2012)
    //GMC's lifetime as a radiation source = 10 t_dyn
    //if (Time > this->BirthTime + 10*this->LifeTime)
    //      result = NO_DEATH;

    //GMC's lifetime as a radiation source = 6 Myrs
    float DensityUnits, LengthUnits, TemperatureUnits, TimeUnits,
      VelocityUnits;
    GetUnits(&DensityUnits, &LengthUnits, &TemperatureUnits,
	     &TimeUnits, &VelocityUnits, Time);
    if (Time > this->BirthTime + 6e6*3.15e7/TimeUnits)
