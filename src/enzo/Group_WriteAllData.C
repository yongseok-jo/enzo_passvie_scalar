...
/* Temporary output of mass leakage from PS field by Jo */
  char *filename_sf = new char[MAX_LINE_LENGTH];
    if (MyProcessorNumber == ROOT_PROCESSOR){
  fprintf(stderr,"Group_WritePSField has started!\n");
  strcpy(filename_sf, "MassLeakageForStarFormation.txt");
  FILE *fp = fopen(filename_sf, "a+");
    if (fp != NULL) 
      fprintf(fp, "%e\n", leakage_sf);
  fclose(fp);
 leakage_sf = 0;
...
/* Temporary output of mass leakage from BH field by Jo */
  char *filename_bh = new char[MAX_LINE_LENGTH];
    if (MyProcessorNumber == ROOT_PROCESSOR){
  fprintf(stderr,"Group_WriteBHField has started!\n");
  strcpy(filename_bh, "MassLeakageForBlackHole.txt");
  FILE *fp = fopen(filename_bh, "a+");
    if (fp != NULL) 
      fprintf(fp, "%e\n", leakage_bh);
  fclose(fp);
 leakage_bh = 0;
