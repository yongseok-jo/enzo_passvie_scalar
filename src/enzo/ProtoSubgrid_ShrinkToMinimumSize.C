...
int ProtoSubgrid::ShrinkToMinimumSize()
{
  ...
  /* For each dim, find first and last location of a non-zero signature. */
	/* To find the first coming (beginning of refinement) cell to be refined */
  ...
  if (MoveFlag) {
    ...
    int size = 1;
    for (dim = 0; dim < MAX_DIMENSION; dim++) {
      NewGridDim[dim] = End[dim] - Start[dim] + 1;   // The new grid dimension which requires refinement.
      size *= NewGridDim[dim];
    }
