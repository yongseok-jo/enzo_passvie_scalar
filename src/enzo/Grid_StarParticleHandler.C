...
int grid::StarParticleHandler(HierarchyEntry* SubgridPointer, int level,
			     float dtLevelAbove) // by Jo
{

#define no_debug_PS // by Jo
#ifdef debug_PS
        if (MyProcessorNumber == ROOT_PROCESSOR){    
	 fprintf(stderr, "Grid level %d, enters Handler!\n", level);
	}
#endif
...
#ifdef debug_PS
        if (MyProcessorNumber == ROOT_PROCESSOR){    
	 fprintf(stderr, "Grid level %d, Handler starts!\n", level);
	}
#endif
...
#ifdef debug_PS
        if (MyProcessorNumber == ROOT_PROCESSOR){    
	 fprintf(stderr, "Grid level %d, UnderSubgrid!\n", level);
	}
#endif
  /* initialize */
...
#ifdef debug_PS
        if (MyProcessorNumber == ROOT_PROCESSOR){    
	 fprintf(stderr, "Grid level %d, indexing!\n", level);
	}
#endif
...
#ifdef debug_PS
        if (MyProcessorNumber == ROOT_PROCESSOR){    
	 fprintf(stderr, "Grid level %d, finish indexing!\n", level);
	}
#endif
  ...
  for (field = 0; field < NumberOfBaryonFields; field++){
    if (FieldType[field] >= ElectronDensity && FieldType[field] <= ExtraType1){ // By Jo
#ifdef EMISSIVITY
      /* 
...
#ifdef debug_PS
        if (MyProcessorNumber == ROOT_PROCESSOR)    
  fprintf(stderr, "StarParticleHandler first indicator ends\n");
#endif
  ...
  if (StarParticleCreation > 0) {
	...
	StarMakerOverDensityThreshold /= m_h / DensityUnits;  
      ...
      for (i = NumberOfNewParticlesSoFar; i < NumberOfNewParticles; i++) {
	tg->ParticleType[i] = NormalStarType;  
#define GMC_TEST_5 //#####  for 121512_dGF-RTF, also check star_maker7.F for additional GMC_TEST changes
#ifdef GMC_TEST_5 // ###### change ParticleType to MBHParticleType
	tg->ParticleType[i] = MBHParticleType;  
	//GMC's lifetime as a radiation source = 6 Myrs -> 100 Myrs ???? by Jo
	tg->ParticleAttribute[1][i] = 100e6*3.15e7/TimeUnits;
#endif /* GMC_TEST_5 */
      }
    } 
   ...
   for (field = 0; field < NumberOfBaryonFields; field++) {
    ...
    } // Loop for summing up updated PS field into new_mass is over 

    if (MyProcessorNumber == ROOT_PROCESSOR){    
      if ( new_mass != old_mass){ 
				fprintf(stderr, "StarParticleHandler first indicator old_mass = %e\n", old_mass);
				fprintf(stderr, "StarParticleHandler first indicator new_mass = %e\n", new_mass);
				fprintf(stderr, "StarParticleHandler first indicator diff_mass = %e\n", old_mass-new_mass);
			}
		}

/* updating mass leakage from star formation by Jo*/
    leakage_sf_tmp += (old_mass - new_mass); 
    //if (MyProcessorNumber == ROOT_PROCESSOR)    
     //fprintf(stderr, "Mass leakage total in Handler: %e\n", leakage_sf_tmp);
//MPI_Allreduce(&mass_leakage, mass_leakage_total, 1, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
...
#ifdef debug_PS
  if (MyProcessorNumber == ROOT_PROCESSOR)    
  fprintf(stderr, "StarParticleHandler third indicator\n");
#endif
  /* ------------------------------------------------------------------- */
...
#ifdef debug_PS
  if (MyProcessorNumber == ROOT_PROCESSOR)    
  fprintf(stderr, "StarParticleHandler forth indicator\n");
#endif
...
// #ifdef GMC_TEST
//       for (i = 0; i < NumberOfParticles; i++) {
// 	if(ParticleNumber[i] == 16411177 || ParticleNumber[i] == 16411390)
// 	  printf("grid::StarParticleHandler: id = %d, mass = %lf\n", 
// 		 ParticleNumber[i], ParticleMass[i]);
//       }
// #endif /* GMC_TEST */
...
#ifdef debug_PS
  if (MyProcessorNumber == ROOT_PROCESSOR)    
  fprintf(stderr, "StarParticleHandler fifth indicator\n");
#endif
...
#ifdef debug_PS
  if (MyProcessorNumber == ROOT_PROCESSOR)    
  fprintf(stderr, "StarParticleHandler sixth indicator\n");
#endif
  for (field = 0; field < NumberOfBaryonFields; field++) {
...
#ifdef debug_PS
  if (MyProcessorNumber == ROOT_PROCESSOR)    
  fprintf(stderr, "StarParticleHandler seventh indicator\n");
#endif
  /* Clean up. */
...
#ifdef debug_PS
  if (MyProcessorNumber == ROOT_PROCESSOR)    
  fprintf(stderr, "StarParticleHandler eighth indicator\n");
#endif
