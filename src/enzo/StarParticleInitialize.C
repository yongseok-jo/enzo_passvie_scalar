			   ...
			   int TotalStarParticleCountPrevious[])
{
  ...
  /* Find MBH particle's position and synchronize */
  for (level = 0; level < MAX_DEPTH_OF_HIERARCHY-1; level++){
    for (Temp = LevelArray[level]; Temp; Temp = Temp->NextGridThisLevel){
      if (Temp->GridData->FindMBHParticles(level, sbuf)==FAIL){            //by Jo
	ENZO_FAIL("Error in grid::FindMBHParticles.");               //
      }                                                              //
    }
  }
  MPI_Allreduce(sbuf, bh_pos, 3, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  //printf("RANK: %d , Position: %lf, %lf, %lf\n", MyProcessorNumber,bh_pos[0],bh_pos[1], bh_pos[2]);
