...
#define GMC_TEST  //#####
#ifdef GMC_TEST  
  BirthTime = 0.5 * (BirthTime + a.BirthTime);
#endif /* GMC_TEST */
  return;
}
...
#ifdef GMC_TEST_6
  return type == a.type && type == MBH && BirthTime > 0 && a.BirthTime > 0;
#else
...
// Usually mass is never copied from the particle to Star,
// but below is reinstated because for GMC_TEST
// particle masses were changed in star_maker and better be propagated into Star routines
// (used in StarParticleFinalize along with UpdatePositionVelocity)
#ifdef GMC_TEST  
void Star::UpdateMass(void)
{
  int i, dim;
  int _id = -1;
  if (CurrentGrid != NULL && type >= 0) { // on local processor and active
    ...
    assert(_id >= 0);
    Mass = CurrentGrid->ParticleMass[_id];
    this->ConvertMassToSolar();
//    if(Identifier == 16411177 || Identifier == 16411390)
//      printf("star::UpdateMass: id = %d, mass = %lf\n", Identifier, Mass);
  }
  return;
}

float Star::BirthTimeSeparation2(Star a) { return powf(BirthTime - a.BirthTime, 2.0f); };

float Star::BirthTimeSeparation2(Star *a) { return this->BirthTimeSeparation2(*a); };
#endif /* GMC_TEST */
...
void Star::CopyFromParticle(grid *_grid, int _id, int _level)
{
  ...
  // below two lines were removed because we want to keep Star->Mass as double 
