...
#define GMC_TEST //#####
#ifdef GMC_TEST 
    // This is needed for GMC where a particle changes its mass in StarParticleHandler
    // between StarParticleInitialize and StarParticleFinalize
    ThisStar->UpdateMass();
#endif /* GMC_TEST */
  }
...
#ifdef GMC_TEST_6
    if (MBHParticleIO == TRUE && ThisStar->ReturnType() == PARTICLE_TYPE_MBH && ThisStar->ReturnBirthTime() < 0) {
#else
