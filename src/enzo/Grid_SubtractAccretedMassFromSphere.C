					 ...
					 int &CellsModified)
{
  ...
  double old_mass, new_mass, increase;
  old_mass = 0;
	new_mass = 0;
  ...
  int SNColourNum, MetalNum, MBHColourNum, Galaxy1ColourNum, Galaxy2ColourNum, ExtraType0Num; 
  int MetallicityField = FALSE;

  if (this->IdentifyColourFields(SNColourNum, MetalNum, MBHColourNum, 
				 Galaxy1ColourNum, Galaxy2ColourNum, ExtraType0Num) == FAIL) {
...
/* Before actual calculation starts, save the old mass of the passive scalar field */
  //old_mass =  BaryonField[ExtrahyType0Num];
  /***********************************************************************
...
//	  printf("grid::SAMFS: increase = %lf\n", increase); 
	...
	leakage_bh_tmp += (old_mass - new_mass)
					*(pow(LengthUnits*CellWidth[0][0],3))
					*DensityUnits; // To track the black hole accretion value by Jo
