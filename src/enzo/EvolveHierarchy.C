...
*/
  ...
  while (!Stop) {
    ...
    /* This allows for enzo to re-set the refine region egdes as MBH particles moves. */
#define GMC_TEST       //
#ifdef GMC_TEST        // by Jo
    int RefineRegionAutoAdjustByMBH = 1;
    if (RefineRegionAutoAdjustByMBH == 1){
      if(AdjustRefineRegionByMBH(MetaData) == FAIL){
...
// Merging all the value from each mpi processors by Jo
    MPI_Allreduce(&leakage_sf_tmp, &leakage_sf, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD); // by Jo
    MPI_Allreduce(&leakage_bh_tmp, &leakage_bh, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD); // by Jo
    leakage_sf_tmp = 0.0;
		leakage_bh_tmp = 0.0;

    if (MyProcessorNumber == ROOT_PROCESSOR){
	fprintf(stderr, "Total Mass Leakage from SF: %e\n", leakage_sf);
	fprintf(stderr, "Total Mass Leakage from BH: %e\n", leakage_bh);
    }
