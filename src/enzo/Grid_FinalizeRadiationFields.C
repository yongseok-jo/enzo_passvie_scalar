...
#define NO_GMC_TEST_3 
#ifdef GMC_TEST_3 //#####
	BaryonField[kphHINum][index] = max(BaryonField[kphHINum][index]/CellVolume, 1e-10*tiny_number);
#else
  ...
  if (RadiativeTransferHydrogenOnly == FALSE)
	...
	for (i = GridStartIndex[0]; i <= GridEndIndex[0]; i++, index++) {
#ifdef GMC_TEST_3 //#####
	  BaryonField[kphHeINum][index] = max(BaryonField[kphHeINum][index]/CellVolume, 1e-10*tiny_number);
	  BaryonField[kphHeIINum][index] = max(BaryonField[kphHeIINum][index]/CellVolume, 1e-10*tiny_number);
#else
