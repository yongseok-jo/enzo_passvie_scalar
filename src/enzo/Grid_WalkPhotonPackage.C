...
#define NO_GMC_TEST_4
#ifdef GMC_TEST_4 //#####
	// Subtract the photons for each i before calculating the next i.
	// While this is a more accurate method, it shouldn't make much difference 
	// if the absorption fraction is not high (i.e. for X-ray photons)
	// This really matters when the photon energy is not high enough 
	// so photons tend to be absorbed very quickly within a few cells from the source
	// By Ji-hoon Kim, Mar. 2012,
        (*PP)->Photons -= dPXray[i];
...
#ifdef GMC_TEST_4 //#####
      // because dP has been subtracted from (*PP)->Photons above already for each i
      (*PP)->Photons += dP;
#endif 
