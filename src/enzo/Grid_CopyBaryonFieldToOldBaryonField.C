...
int grid::CopyBaryonFieldToOldBaryonField()
{
  ...
  if( (SelfGravity || UniformGravity || PointSourceGravity) && (NumberOfBaryonFields > 0) ) {
    ...
    for(field = 0; field < GridRank; field++) {
     ...
     //fprintf(stderr,"right before OldPS!\n"); // by Jo
     //if(OldPSFieldForStarFormation == NULL){
      // fprintf(stderr,"OldPS initialize!\n"); // by Jo
//      OldPSFieldForStarFormation = new float[size];
//}
//	for (i = 0; i < size; i++)
 //         OldPSFieldForStarFormation[i] = 0;
 //    if(NewPSFieldForStarFormation == NULL){
  //    NewPSFieldForStarFormation = new float[size];
  //    }
//	for (i = 0; i < size; i++)
 //         NewPSFieldForStarFormation[i] = 0;
