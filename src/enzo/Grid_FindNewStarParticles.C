...
#define GMC_TEST //##### identify as Stars only within or outside a certain radius from a center
#ifdef GMC_TEST
	...
	if (ParticleType[i] == PARTICLE_TYPE_MBH &&
	    ...
	    sqrt(result) > 0.00005)// when at DD0261 by Jo tentative
	  // sqrt(result) < -0.0025) 
	  goto next;
...
//      initial_angle = 135; // in degree, initially this should give (0.4950, 0.5050, 0.5000)
	...
	if (ParticleType[i] == PARTICLE_TYPE_MBH &&
	    //sqrt(result) > 0.002
	    sqrt(result) > 0.0005)// when at DD0410 by Jo tentative
	  ) 
	  goto next;
