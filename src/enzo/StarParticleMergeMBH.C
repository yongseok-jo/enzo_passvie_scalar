...
/  star particles. ###### I should modify
/
************************************************************************/
...
#define GMC_TEST //#####
#ifdef GMC_TEST
  //  float tmerge2 = powf(30000 * 3.15e7 / TimeUnits, 2.0f);
  float tmerge2 = powf(100000 * 3.15e7 / TimeUnits, 2.0f);
#endif
  ...
  for (ThisStar = AllStars; ThisStar; ThisStar = ThisStar->NextStar) {
...
#ifdef GMC_TEST // merge if within 30000 or 100000 years apart in creation time
	  ThisStar->BirthTimeSeparation2(OtherStar) < tmerge2) {
#else
	  ThisStar->RelativeVelocity2(OtherStar) < vcirc2) {
#endif /* GMC_TEST */
	...
	if (MyProcessorNumber == ROOT_PROCESSOR) {
	  fprintf(stdout, "Merging stars %"ISYM" and %"ISYM"\n", ThisStar->ReturnID(),
		  OtherStar->ReturnID());  
	  // fprintf(stdout, "Candidates: mergable = %d, Sep2 = %g, rmerge2 = %g, BTSep2 = %g, tmerge2 = %g, RelVel2 = %g, vcirc2 = %g \n", 
	  // 	  ThisStar->MergableMBH(OtherStar), ThisStar->Separation2(OtherStar), rmerge2,
	  // 	  ThisStar->BirthTimeSeparation2(OtherStar), tmerge2,
	  // 	  ThisStar->RelativeVelocity2(OtherStar), vcirc2); 
	}
